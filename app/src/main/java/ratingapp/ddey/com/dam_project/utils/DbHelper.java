...
public class DbHelper extends SQLiteOpenHelper {
    ...
    public List<Result> getAllResults() {
        List<Result> resultList = new ArrayList<>();
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor c = db.query(RESULT_TABLE, new String[]{COLUMN_ID_RESULT, COLUMN_QUIZZ_NAME, COLUMN_STUDENT_NAME, COLUMN_SCORE, COLUMN_DATETIME}, null, null, null, null, null);
        while (c.moveToNext()) {

            int index = c.getColumnIndex(COLUMN_ID_RESULT);
            long idResult = c.getLong(index);

            index = c.getColumnIndex(COLUMN_QUIZZ_NAME);
            String quizzName = c.getString(index);
    ...
    /*
        UPDATES QUIZ + QUESTIONS + ANSWERS
     */
    ...
    public void updateQuiz(Quiz quiz) {
        ...
        cv.put(COLUMN_TITLE, quiz.getTitle());
        cv.put(COLUMN_DESCRIPTION, quiz.getDescription());
        cv.put(COLUMN_VISIBILITY, quiz.getVisibility());
        cv.put(COLUMN_ACCESS_CODE, quiz.getAccessCode());
        cv.put(COLUMN_CATEGORY, quiz.getCategory());

        db.update(QUIZ_TABLE, cv, COLUMN_ID_QUIZ + " = ? ", new String[]{String.valueOf(quiz.getIdQuizz())});
        db.close();
    }
    ...
    public void updateQuestion(Question question) {
        ...
        ContentValues cv = new ContentValues();
        cv.put(COLUMN_ANSWER_TIME, question.getAnswerTime());
        cv.put(COLUMN_TEXT, question.getQuestionText());

        db.update(QUESTION_TABLE, cv, COLUMN_ID_QUESTION + " = ? ", new String[]{String.valueOf(question.getIdQuestion())});
        db.close();
    }
    ...
    public void updateAnswer(Answer answer) {
        ...
        cv.put(COLUMN_IS_CORRECT, answer.isCorrect());

        db.update(ANSWER_TABLE, cv, COLUMN_ID_ANSWER + " = ? ", new String[]{String.valueOf(answer.getIdAnswer())});
        db.close();
    }
    ...
    public void deleteUser(long idUser) {
        ...
        db.delete(USER_TABLE, COLUMN_ID_USER + " = ?", new String[]{String.valueOf(idUser)});
        db.close();
    }
    ...
    public void deleteResult(Result r) {
        if (r != null) {
            SQLiteDatabase db = this.getWritableDatabase();
            db.delete(RESULT_TABLE, COLUMN_ID_RESULT + " = ?", new String[]{String.valueOf(r.getIdResult())});
            db.close();
        }
    }
    ...
    public void deleteQuestionSolely(Question question) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(QUESTION_TABLE, COLUMN_ID_QUESTION + " = ?", new String[]{String.valueOf(question.getIdQuestion())});
        db.close();
        for (Answer a : question.getAnswersList()) {
            deleteAnswer(a, question.getIdQuestion());
        }
    ...
    public void deleteAnswer(Answer answer, long idQuestion) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(ANSWER_TABLE, COLUMN_ID_QUESTION + " = ?", new String[]{String.valueOf(idQuestion)});
        db.close();
    }
}
